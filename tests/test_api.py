...
def test_get_all_meme_negative(all_meme):
    all_meme.get_all_meme()
    all_meme.check_status_code(401)


def test_create_meme_success(post_new_meme, auth_token,
                             yandex_taxi_payload,
                             delete_meme_endpoint):
    post_new_meme.post_meme(
        body=yandex_taxi_payload, token=auth_token["token"]
    )
    post_new_meme.check_status_code(200)
    post_new_meme.check_created_meme_fields(
        expected_body=yandex_taxi_payload,
        expected_user=auth_token["user"]
    )
    created_id = post_new_meme.post_id
    delete_meme_endpoint.delete_meme(id=created_id,
                                     token=auth_token["token"])


@pytest.mark.parametrize('data', invalid_payload_data)
def test_create_meme_without_required_field(data, post_new_meme,
                                            auth_token):
    post_new_meme.post_meme(
        body=data, token=auth_token["token"])
    post_new_meme.check_status_code(400)


def test_get_one_meme(created_meme_id, get_meme_id, auth_token):
    get_meme_id.get_one_meme(id=created_meme_id,
                             token=auth_token["token"]
                             )
...
def test_update_meme(created_meme_id, auth_token,
                     updated_meme_payload, updated_meme,):
    updated_meme.put_meme(
        body=updated_meme_payload,
        id=created_meme_id,
        token=auth_token["token"]
    )
    updated_meme.check_status_code(200)
    updated_meme.check_update_meme_fields(
        expected_body=updated_meme_payload,
        expected_user=auth_token["user"]
    )


def test_update_meme_without_token(updated_meme, created_meme_id,
                                   updated_meme_payload):
    updated_meme.put_meme(body=updated_meme_payload,
                          id=created_meme_id, token=None)
    updated_meme.check_status_code(401)


def test_delete_meme_success(created_meme_id,
                             delete_meme_endpoint,
                             auth_token,
                             get_meme_id):
    delete_meme_endpoint.delete_meme(id=created_meme_id,
                                     token=auth_token["token"])
...
def test_delete_meme_twice(created_meme_id,
                           delete_meme_endpoint,
                           auth_token):
    delete_meme_endpoint.delete_meme(id=created_meme_id,
                                     token=auth_token["token"])
